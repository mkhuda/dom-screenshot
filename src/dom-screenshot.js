(function (global) {
  ...
  var images = newImages();
    ...
    /**
    * @typedef {object} Options - 'Option' specialType
    * @property {Function=} filter - Should return true if passed node should be included in the output
    *          (excluding node means excluding it's children as well). Not called on the root node.
    * @property {String=} bgcolor - color for the background, any valid CSS color value.
    * @property {Number=} width - width to be applied to node before rendering.
    * @property {Number=} height - height to be applied to node before rendering.
    * @property {Object=} style - an object whose properties to be copied to node's style before rendering.
    * @property {Number=} quality - a Number between 0 and 1 indicating image quality (applicable to JPEG only),
    defaults to 1.0.
    * @property {String=} imagePlaceholder - dataURL to use as a placeholder for failed images, default behaviour is to fail fast on images we can't fetch
    * @property {Boolean} [cacheBust=false] - set to true to cache bust by appending the time to the request url
    */
    ...
    function processClone(original, originalClone) {
      ...
      function renderVideo() {
        if (clone instanceof HTMLVideoElement) {
          ...
          var ratio = Math.max(
            original.videoWidth / canvas.width,
            original.videoHeight / canvas.height
          );
